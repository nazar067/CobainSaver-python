...
async def download_photo_gallerydl(bot: Bot, url: str, chat_id: int, dp: Dispatcher, business_connection_id, msg_id) -> List[str]:
    try:
        pool = dp["db_pool"]
        ...
        cmd = [
            ...
            "-o", "directory=",             # пустая директория = без подпапок
            "-o", f"filename={uniq_id}_{{num}}.{{extension}}",  # имя: порядковый номер
            url,
        ]
